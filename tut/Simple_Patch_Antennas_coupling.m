...
STRT2 = [0 0 -50]; %starting point for 2nd antena
start = [STRT2(1)-patch.width/2 STRT2(2)-patch.length/2 STRT2(3)-substrate.thickness];
stop  = [ STRT2(1)+patch.width/2  STRT2(2)+patch.length/2 STRT2(3)-substrate.thickness];
CSX = AddBox(CSX,'patch2',9,start,stop); % add a box-primitive to the metal property 'patch'

% Substate 2
start = [STRT2(1)-substrate.width/2 STRT2(2)-substrate.length/2 STRT2(3)-0];
stop  = [STRT2(1)+substrate.width/2 STRT2(2)+substrate.length/2 STRT2(3)-substrate.thickness];
...
start(3)=STRT2(3)-0;
stop(3) =STRT2(3)-0;
CSX = AddBox(CSX,'gnd2',8,start,stop);

% Apply the Excitation & Resist as a Current Source
start = [STRT2(1)+feed.pos STRT2(2)+0 STRT2(3)-0];
stop  = [STRT2(1)+feed.pos STRT2(2)+0 STRT2(3)-substrate.thickness];
[CSX port{2}] = AddLumpedPort(CSX, 4 ,2 ,feed.R, start, stop, [0 0 -1], false);



% Finalize the Mesh
% detect all edges except of the patch
mesh = DetectEdges(CSX, mesh,'ExcludeProperty','patch','patch2');
% detect and set a special 2D metal edge mesh for the patch
mesh = DetectEdges(CSX, mesh,'SetProperty','patch','patch2','2D_Metal_Edge_Res', c0/(f0+fc)/unit/50);
