...
%   difference less than "eee"
    ...
    for j=1:length (points) %............with any other cell in the row !!!
        for k = 1 : size (points,1) % for each row in "points"  % for each coordinate (x, y or z)

            if exist('feed','var')                      % if 'feed' argument is set. Check if the value is similar to any of the corresponding "feed-coordinates"
                diffs = abs(points(k,:) - feed(k,:));   % calculate a matrix with the differences between eace element of "feed" and each element of "points"
                ind = diffs < eee;                      % find the position of the values
                for m=1:size(feed,2)                    % for each value in "feed" matrix
                   points(k,ind(m,:))= feed(k,m);       % replace the values with those in "feed" matrix
                end
            end
